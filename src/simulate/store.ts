...
export function configureDemoDbPath(filePath: string): void {
  closeDemoDb();
  configuredPath = path.resolve(filePath);
}

export function demoDbPath(): string {
  return configuredPath ?? path.resolve(process.cwd(), DEMO_DB_FILENAME);
}
