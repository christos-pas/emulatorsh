...
import { closeConfirmationItems, isCloseRequest } from "./devices/close.js";
import { runningSummary, type Runtime } from "./runtime.js";
import type { FormFactor, MenuItem, SystemImage } from "./types.js";

export function startedMessage(detail: string, simulate = isSimulate()): string {
...
export function closedMessage(action: "suspend" | "terminate", name: string, simulate = isSimulate()): string {
  const note = simulate ? ` ${ORANGE}${SIMULATE_NOTE}${RESET}` : "";
  const text =
    action === "suspend"
