...
import { EMULATOR_LOG } from "../constants.js";
import { appleDeviceLabel, appleRuntimeFromKey } from "../ios/runtime.js";
import type { ExecOutputError } from "../types.js";
import { parseAvdApi } from "../android/format.js";
import { abiFromSysdir, specFromSysdir, specToPackage, specToSysdir } from "../android/specs.js";
import {
  DEMO,
  allDemoImageSpecs,
  demoProfiles,
...
import { DEMO_PIDS, DEMO_PLATFORM } from "./constants.js";
import {
  MOCK_ADB,
  MOCK_AVD_HOME,
  MOCK_AVDMANAGER,
...
import { closeFakeEmulator, fakeEmulatorKind, openFakeEmulator } from "./fake-window.js";
import { addStoredSdk, listStoredDevices, listStoredSdks, upsertStoredDevice } from "./store.js";
...
export function mockSpawn(
  ...
  if (tool === "emulator") {
    ...
    if (avd) {
      ...
      openFakeEmulator(
        fakeEmulatorKind(avd, androidDevices().find((device) => device.name === avd)),
        avd,
        avd,
      );
    }
